...
3. **Pairing code.** In the Home Screen app tap *Pair* and type the six‑digit code shown on the PC. A new code appears every 60 seconds (the previous one is accepted for a few more seconds while you finish typing), five wrong attempts rotate it early, and codes only exist while that window is open. The setup page has a *Test the connection* button that tells you whether the certificate is fully trusted yet.

Photos and videos are never re-encoded: the phone encrypts the file it is given and the PC stores exactly those bytes (the app asks iOS for the originals, so HEIC and HEVC/MOV pass through untouched; if iOS offers a size, choose *Actual Size*). Previewing HEIC/HEVC on Windows needs Microsoft's HEIF/HEVC extensions; without them the file is still stored and can be saved as a copy.
...
- `test.cmd` — runs the test program with temporary data: encryption, tamper detection, migration, save failures, autosave, search, trash policy, bulk actions, attachments, tray behaviour and UI layout, plus a Node script that plays the phone (pairing, sync, conflicts, byte-identical files).
